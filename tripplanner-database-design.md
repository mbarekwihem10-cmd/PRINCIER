...
**Index** : `(token)` unique pour la résolution du lien d'invitation ; `(trip_id, email)` pour éviter les doublons d'invitation active.
...
**Index** : `(trip_id, type)` pour le filtrage du tableau de bord ; `UNIQUE (provider, provider_reference)` empêche la double-création d'une réservation suite à un retry réseau (idempotence).
...
flight_passengers(id uuid PK, flight_booking_id FK NOT NULL ON DELETE CASCADE,
  trip_member_id FK trip_members NOT NULL ON DELETE RESTRICT,
  first_name varchar(100) NOT NULL, last_name varchar(100) NOT NULL, seat varchar(10), created_at)
```
**`trip_member_id ON DELETE RESTRICT`** : un passager nommé sur un billet ne peut pas voir sa référence supprimée silencieusement — retirer un membre d'un voyage avec un billet actif doit être bloqué ou géré explicitement par l'application, jamais en cascade.
...
```
**Index sur `notifications`** : `(user_id, read_at)` — la requête "notifications non lues de l'utilisateur" est appelée à chaque ouverture d'app, doit rester rapide même avec un historique important.
...
| `flight_passengers.trip_member_id → trip_members` | RESTRICT | Empêche la perte silencieuse d'un passager nommé |
...
Le schéma est en **3NF par défaut**, avec trois dénormalisations délibérées, chacune documentée à sa table :
...
Ces trois cas suivent la même règle : **on dénormalise uniquement quand la normalisation stricte casserait soit la performance d'une requête critique, soit une exigence métier d'immuabilité — jamais par confort de développement.**
